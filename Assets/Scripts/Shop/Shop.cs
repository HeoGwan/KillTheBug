...
public class Shop : MonoBehaviour
{
    ...
    public GameObject ToolObjs; // 상점에 배치되는(아이템을 담는) 부모 오브젝트
    public GameObject BuyItemObj;
    public ShopItem[] Items; // 아이템 리스트
    ...
    public void UnLock(TOOL tool)
    {
        ...
        shopItems[(int)tool].transform.GetChild(1).gameObject.SetActive(false); // Lock 이미지 비활성화
        shopItems[(int)tool].GetComponent<Button>().onClick.RemoveAllListeners(); // 버튼 이벤트 제거
    }

    public void Lock(TOOL tool)
    {
        shopItems[(int)tool].GetComponent<ShopItem>().Lock();
        shopItems[(int)tool].transform.GetChild(1).gameObject.SetActive(true); // Lock 이미지 활성화
